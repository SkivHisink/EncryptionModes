...
class CipherBlockChaining final : public EncryptionBase
{
	std::string key_gen() override;
public:
	std::string initial_vector_gen(size_t partition_size) override;
	explicit CipherBlockChaining(std::function<char(char, char)>new_encrypt_method,
